...
<div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', background: theme.canvas, color: theme.text, padding: 32, borderRadius: 6, border: `1px solid ${theme.border}` }}>
  ...
  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, marginTop: 26 }}>
    {[
    ...
    ].map(([label, value, color]) => <div key={label} style={{ display: 'flex', flexDirection: 'column', width: 358, height: 108, padding: '14px 20px', borderRadius: 6, background: theme.subtle }}>
      ...
      <div style={{ display: 'flex', fontSize: 19, marginTop: 4, color: theme.muted }}>{label}</div>
    </div>)}
  </div>
</div>
```
...
<div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', background: theme.canvas, color: theme.text, padding: 32, borderRadius: 6, border: `1px solid ${theme.border}` }}>
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
    ...
    <div style={{ display: 'flex', fontSize: 18, color: theme.muted }}>PAST 90 DAYS</div>
  </div>
  <div style={{ display: 'flex', marginTop: 12, fontSize: 21, color: theme.muted }}>{stats.totals.contributions90.toLocaleString('en-US')} contributions · {stats.totals.activeDays30}/30 recent days active</div>
  <div style={{ display: 'flex', alignItems: 'flex-end', gap: 12, height: 170, marginTop: 24, borderBottom: `1px solid ${theme.border}`, paddingBottom: 10 }}>
    {stats.weekly.map((week, i) => <div key={week.from} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', width: 45, height: '100%' }}>
      <div style={{ display: 'flex', fontSize: 16, marginBottom: 7, color: theme.muted }}>{week.count}</div>
      <div style={{ display: 'flex', width: 36, height: week.count === 0 ? 1 : Math.max(3, Math.round(week.count / Math.max(1, ...stats.weekly.map(w => w.count)) * 115)), background: i === stats.weekly.length - 1 ? theme.greenStrong : theme.green, borderRadius: '5px 5px 0 0' }} />
    </div>)}
  </div>
  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 12, fontSize: 18, color: theme.muted }}>
...
<div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', background: theme.canvas, color: theme.text, padding: 32, borderRadius: 6, border: `1px solid ${theme.border}` }}>
  ...
  <div style={{ display: 'flex', fontSize: 21, marginTop: 12, color: theme.muted }}>{stats.totals.activeRepositories === null ? 'Repository detail unavailable with the current credential' : `${stats.totals.activeRepositories} active repositories in the past 365 days`}</div>
  <div style={{ display: 'flex', fontSize: 18, marginTop: 24, color: theme.muted }}>TOP LANGUAGES · BY ACTIVE REPOSITORY COUNT</div>
  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginTop: 16 }}>
    {stats.languages === null ? <div style={{ display: 'flex', fontSize: 21, color: theme.muted }}>Private activity is included above as anonymous totals.</div> : stats.languages.length === 0 ? <div style={{ display: 'flex', fontSize: 21 }}>No language data in this period.</div> : stats.languages.map(lang => <div key={lang.name} style={{ display: 'flex', alignItems: 'center', padding: '12px 16px', background: theme.subtle, borderRadius: 6, fontSize: 21 }}>
